...
### pseudo element
For each of the property generated into pseudo class (font-family, font-size, font-style, font-variant, font-weight, text-align, text-decoration, text-transform) you can generate pseudo element helper class (:hover, :focus, :active).

You must turn to `true` the variables for each property on this model:
```
${property}__pseudo-class--{pseudo-element}: true !default;
```

You can also change the prefix on the class generated by editing the variables on this model:
```
${property}__pseudo-class--{pseudo-element}-prefix;

// For example
$font-family__pseudo-class--hover-suffix: '--hover';
```


### add prefix to helper classes
If you want to, you can add a prefix to all the class generated by this library. You just have to reset the sass variable `$typo-helper__prefix`.

```
$typo-helper__prefix: 'prefix-';

// This will generate for example this class selector
